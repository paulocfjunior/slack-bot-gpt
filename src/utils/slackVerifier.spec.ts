...
import { verifySlackSignature, validateTimestamp } from './slackVerifier';
...
describe('slackVerifier', () => {
  ...
  describe('verifySlackSignature', () => {
    ...
    it('should return false for old timestamps', () => {
      ...
      const result = verifySlackSignature(body, signature, oldTimestamp, signingSecret);
    ...
    it('should return false for invalid signature', () => {
      ...
      const result = verifySlackSignature(body, signature, timestamp, signingSecret);
    ...
    it('should handle complex JSON body', () => {
      const timestamp = Math.floor(Date.now() / 1000).toString();
      const jsonBody = JSON.stringify({ event: { type: 'message', text: 'Hello!' } });
