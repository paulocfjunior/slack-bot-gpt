...
describe('Thread Storage', () => {
  ...
  it('should load threads from file', async () => {
    ...
    (fs.readFileSync as jest.MockedFunction<typeof fs.readFileSync>).mockReturnValue(mockData);

    const threadStorage = new ThreadStorage();

    expect(fs.readFileSync).toHaveBeenCalledWith(expect.stringContaining('user-threads.json'), 'utf8');
  ...
  it('should save threads to a file', async () => {
    ...
    expect(fs.writeFileSync).toHaveBeenLastCalledWith(expect.stringContaining('user-threads.json'), expect.stringContaining('\"test\": \"test\"'));
  });
  ...
  it('should return all threads', async () => {
    ...
    expect(threadStorage.getAll()).toEqual({ test1: 'test', test2: 'test', test3: 'test' });
  });
